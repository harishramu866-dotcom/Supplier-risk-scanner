...
from risk_scanner.extraction import extract_category_findings
from risk_scanner.validation import is_valid_category_result


def _latest_output_file() -> Path:
    candidates = sorted(Path("output").glob("*.json"), key=lambda p: p.stat().st_mtime)
    if not candidates:
        raise FileNotFoundError("no files found in output/ — run scripts/run_search_agent.py first")
    return candidates[-1]
